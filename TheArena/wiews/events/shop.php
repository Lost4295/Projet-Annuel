...
<?php if (isConnected() && ($user['id'] == $event['manager_id']) && isset( $event['shop_id'] ) ) { ?>
    <a title="Créer un article" href='/event/shop/create/item?shop=<?php echo $event['shop_id'] ?>'>Créer un article</a>
<?php } ?>

<?php if (count($items)==0 && !isset( $event['shop_id'] )){?>
    <a title="Créer un shop" href='/event/shop/create/shop?event=<?php echo $event['id']?>'>Créer un shop</a>
<?php ;}
?>
</div>
