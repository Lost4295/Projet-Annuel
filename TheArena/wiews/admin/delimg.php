...
if (isset($_GET['src'])) {
    ...
    if ($_SESSION['chosen_img']== $imagePath){
     $_SESSION['message'].="Avertissement : l'image effacée était celle utilisée pour le captcha. Une image au hasard à été choisie. S'il n'y en a pas, retour à l'image par défaut.";
     if (empty($images)){
     $_SESSION['chosen_img']=$defImages[array_rand($defImages)];
     } else {
          $_SESSION['chosen_img']=$images[array_rand($images)];
     }
    }
